								...
								success: function(pluginHandle) {
									...
									$('#start').removeAttr('disabled').html("Stop")
										.click(function() {
											...
											deleteRoomOnLeave(janus.destroy);
										});
								},
		...
		success: function(data) {
			...
			for (let item of data.list) {
				...
				if (roomToIDMapping.has(roomName)) {
					...
					while (roomToIDMapping.has(roomName + ' (' + idx + ')')) {
						idx++;
					}
...
function deleteRoomOnLeave(callback) {
	...
	sfutest.send({
		...
		success: function(data) {
			console.log("on leave there are " + data.participants?.length ?? 0  + " participants");
